  ...
  return (
    <div className="language-switcher">
      <Select value={currentLocale} onValueChange={handleLanguageChange} disabled={isChanging}>
        ...
        <SelectContent className="language-select-content" align="end">
          {languages.map((lang) => (
            <SelectItem key={lang.lang} value={lang.lang} className="language-select-item">
              <motion.div className="language-item-content" initial={{ x: -10, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.2 }}>
                <span className="language-item-code">{lang.lang}</span>
                <span className="language-item-name">{lang.language}</span>
              </motion.div>
            </SelectItem>
          ))}
        </SelectContent>
